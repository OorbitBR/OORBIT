// TODO: try to remove mention of econtrol on the environment component
/*OORBITfunction
...
removed logPreset()
added this.dressing.setAttribute('shadow','cast',this.environmentData.shadowDressing @ updateDressing()
rewrote createStars() and setStars()
added subsea preset
*/
	...
	if (typeof AFRAME === 'undefined') {
      ...
      AFRAME.registerComponent('environment', {
        schema: {
          ...
          preset: {default: 'default', oneOf: ['none', 'default', 'subsea', 'contact', 'egypt', 'checkerboard', 'forest', 'goaland', 'yavapai', 'goldmine', 'arches', 'threetowers', 'poison', 'tron', 'japan', 'dream', 'volcano', 'starry', 'osiris']},
          ...
          // OORBIT 1.3.1
          shadowDressing: {default: false},
          stageSize: {default: 200},
          groundDetail: {default: 64},
          skyDetail: {default: 64},
          stars: {default: 2000}
          //####//
        },
        ...
        presets: {
          ...
          'subsea': {active: true, seed: 11, skyType: 'gradient', skyColor: '#8baaea', 
          horizonColor: '#0e0ec4', lighting: 'distant', lightPosition: { x: 0.5, y: 1, z: 0}, 
          fog: 0.9, flatShading: true, playArea: 1, ground: 'hills', 
          groundYScale: 9.76, groundTexture: 'walkernoise', groundColor: '#916d5f', groundColor2: '#694439', 
          dressing: 'stones', dressingAmount: 10, dressingColor: '#494949', dressingScale: 0.1, 
          dressingVariance: { x: 0.2, y: 0.1, z: 0.2}, dressingUniformScale: true, dressingOnPlayArea: 1, 
          grid: 'none', gridColor: '#239893', shadow: true}
        },
        ...
        init: function () {
          ...
          this.STAGE_SIZE = this.data.stageSize; //OORBIT 1.3.1//
        ...
        update: function (oldDataNonPreset) {
          ...
          if (this.el.hasAttribute("econtrol") && this.el.getAttribute("econtrol").universe) {
      ...
      */
          ...
          // OORBIT 1.3.1
          this.dressing.setAttribute('shadow','cast',this.environmentData.shadowDressing)
          //####//
        },
  
        // initializes the BufferGeometry for the stars
        createStars: function(h=0 /*OORBIT 1.3.1*/) {
          var numStars = this.data.stars;
          var geometry = new THREE.BufferGeometry();
          var positions = new Float32Array( numStars * 3 );
          var radius = this.STAGE_SIZE - 1;

          if (this.el.hasAttribute("econtrol") && this.el.getAttribute("econtrol").skyradius) {
            radius = this.el.getAttribute("econtrol").skyradius - 1;  
          }

          var v = new THREE.Vector3();
          for (var i = 0; i < positions.length; i += 3) {
            v.set(this.random(i + 23) - 0.5, this.random(i + 24) - h, this.random(i + 25) - 0.5);
            v.normalize();
        ...
        setStars: function (numStars,h /*OORBIT 1.3.1*/) {
          if (!this.stars){
            this.stars = document.createElement('a-entity');
            this.stars.classList.add('environment')
            this.stars.id= 'stars';
            this.createStars(h);
            if (this.el.components.econtrol) this.el.components.econtrol.stars = this.stars;
            this.el.appendChild(this.stars);
          }
          numStars = Math.floor(Math.min(this.data.stars, Math.max(0, numStars)));
      ...
      AFRAME.registerShader('skyshader', {
        schema: {
          ...
          topColor: {type: 'color', default: '1 0 0', is: 'uniform'}, //placeholder to remove warning
          bottomColor: {type: 'color', default: '0 0 1', is: 'uniform'}, //placeholder to remove warning
          fog: {default: true}, //placeholder to remove warning
        },
      ...
      AFRAME.registerShader('gradientshader', {
        schema: {
          ...
          color: {type: 'color', default: '#fff', is: 'uniform' }, //placeholder to remove warning
          fog: {default: true}, //placeholder to remove warning
          sunPosition: { type: 'vec3', default: {x: 0, y: 0, z: -1}, is: 'uniform' }, //placeholder to remove warning
        },
