...
//OORBIT let PHYSX = require('./physx.release.js');
...
AFRAME.registerSystem('physx', {
  schema: {
    ...
    // OORBIT show bouding boxes and axles
    debug: {default:false}  
  ...
  init() {
    ...
    this.el.addEventListener('inspectortoggle', (e) => {
      ...
      if (e.detail === true){
        ...
        this.running = true
        o.sceneEl.querySelectorAll('[physx-body').forEach(function (element) {
          element.components['physx-body'].resetBodyPose()
        });
      }
    })
  },
...
AFRAME.registerComponent('physx-body', {
  ...
  schema: {
    ...
    speculativeCCD: {default: true},

  },
  events: {
    stateadded: function(e) {
      if (e.detail === 'grabbed' && this.data.type !== 'static') {
    ...
    contactbegin: function(e) {
        ...
        if (this.el.classList.contains('finger-collider')) {
            ...
            if (hitel.hasAttribute('grabbable')) {
                hitel.setAttribute("gltf-material__hitel", 'opacity',0.6)

                if (hitel.getAttribute('physx-body').type == 'dynamic') {
                    this.rigidBody.setRigidBodyFlag(PhysX.PxRigidBodyFlag.eKINEMATIC, true)
                }
  ...
  createGeometry(o) {
    if (o.el.hasAttribute('geometry'))
    {
      ...
      switch(geometry.primitive)
      {
        ...
        case 'box':
            if(this.system.data.debug && !this.debug) this.createDebugBox(geometry.width, geometry.height, geometry.depth)  
            return new PhysX.PxBoxGeometry(geometry.width / 2 * scale.x,geometry.height / 2 * scale.y,geometry.depth / 2 * scale.z)
        default:
  ...
  createDebugSphere(r){
    ...
    el.setAttribute('radius',r * 1.001)
    el.setAttribute('physx-no-collision','')  
  ...
  createDebugBox(x,y,z){
    ...
    el.setAttribute('width', x * 1.001)
    el.setAttribute('height', y * 1.001)
    el.setAttribute('depth', z * 1.001)
    el.setAttribute('physx-no-collision','')  
    el.setAttribute('material',{
        wireframe: true,
        color: 'red'
