...
/***/ (function(module, exports) {
	...
	AFRAME.registerComponent('environment', {
	  ...
	  init: function () {
		...
		this.STAGE_SIZE = this.data.stageSize; //OORBIT//
	    ...
	    this.hemilight.setAttribute('light', {
	      type: 'hemisphere',
	      color: '#CEE4F0',
	      intensity: (this.intensity || 0.4) /* OORBIT */
	    });
	    ...
	    this.sunlight.setAttribute('light', {intensity: (this.intensity || 0.6) /* OORBIT */});
	    ...
	    if (this.sunlight) {
	      ...
	      if (skyType != 'atmosphere') {
	        ...
	        this.hemilight.setAttribute('light', {
	          ...
	          'intensity': this.intensity /* OORBIT */
	        });
	        this.sunlight.setAttribute('light', {'intensity': this.intensity /* OORBIT */});
	      }
	      else {
	        this.hemilight.setAttribute('light', {
	          'color': '#CEE4F0',
	          'intensity': (.1 + this.intensity * sunPos.y * ((sunPos.y < 0) ? 0 : 1)) /* OORBIT */
	        });
	        this.sunlight.setAttribute('light', {'intensity': (.1 + this.intensity * sunPos.y) * ((sunPos.y < 0) ? 0 : 1) /* OORBIT */});
	      }
	  ...
	  createStars: function(h=0 /*OORBIT */) {
	    var numStars = 2000;
	  ...
	  setStars: function (numStars,h /*OORBIT */) {
	    if (!this.stars){
	      this.stars = document.createElement('a-entity');
	      this.stars.id= 'stars';
	      this.createStars(h);
		  if (this.el.components.econtrol) this.el.components.econtrol.stars = this.stars; /*OORBIT */
	...
	AFRAME.registerShader('skyshader', {
	  schema: {
		...
		topColor: {type: 'color', default: '#fff', is: 'uniform'}, //placeholder to remove warning
		bottomColor: {type: 'color', default: '#fff', is: 'uniform'}, //placeholder to remove warning
