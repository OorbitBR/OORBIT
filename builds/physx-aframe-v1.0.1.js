...
AFRAME.registerSystem('physx', {
  ...
  startPhysXScene() {
    if (dev) console.info("Starting PhysX scene")
...
AFRAME.registerComponent('physx-material', {
  schema: {
    ...
    collidesWithLayers: {default: [1], type: 'array'}, // [1,2,3,4] OORBIT
...
AFRAME.registerComponent('physx-body', {
  ...
  schema: {
    ...
    //oorbit
    //shape: {default: 'hull', oneOf: ['hull', 'mesh']},
  },
  events: {
    ...
    contactbegin: function(e) {
        ...
        let hitel = e.detail.otherComponent.el;
        
        if (this.el.classList.contains('finger-collider')) {
            //if (hitel.wasDynamic) return
            
            this.el.parentElement.components.ccontrol.grabAnchor.emit( 'raycaster-intersection-filtered', {
                els: hitel
            }) 
            if (hitel.hasAttribute('grabbable') && hitel.getAttribute('physx-body').type == 'dynamic') {
                //hitel.emit('changetype')
                
                //this.oldType = hitel.getAttribute('physx-body').type;
                //hitel.setAttribute('physx-body','type','kinematic')

                //this.rigidBody.setRigidBodyFlag(PhysX.PxRigidBodyFlag.eKINEMATIC, true)  
            }
        }
        
        if (hitel.hasAttribute('collidable')) hitel.emit('grab-start', {hand: e.target})
    ...
    contactend: function(e) {
        ...
        //TODO: is it working?
        //else if (hitel.hasAttribute('grabbable') && hitel.getAttribute('physx-body').type != 'kinematic'){
            //this.rigidBody.setRigidBodyFlag(PhysX.PxRigidBodyFlag.eKINEMATIC, true)      
        //} 
        
        if (this.el.classList.contains('finger-collider')) {
            this.el.parentElement.components.ccontrol.grabAnchor.emit('raycaster-intersection-cleared', {
                clearedEls: hitel
            })
        }
        
        if (hitel.hasAttribute('collidable')) hitel.emit('grab-end', {hand: e.target})
    },
    changetype: function() {
  ...
  createGeometry(o) {
    if (o.el.hasAttribute('geometry'))
    {
      ...
      {
        case 'sphere':
            let r = geometry.radius * this.el.object3D.scale.x;
            if(this.system.data.debug && !this.debug) this.createDebugSphere(r)    
          return new PhysX.PxSphereGeometry(r * 0.98)
        case 'box':
            let x = geometry.width / 2 * this.el.object3D.scale.x;
            let y = geometry.height / 2 * this.el.object3D.scale.y;
            let z = geometry.depth / 2 * this.el.object3D.scale.z;
            if(this.system.data.debug && !this.debug) this.createDebugBox(x,y,z)  
          return new PhysX.PxBoxGeometry(x, y, z)
        default:
            this.createConvexMeshGeometry(o.el.getObject3D('mesh'));  
            //this.createDebugHull(o.el.getObject3D('mesh'))
  ...
  createDebugBox(x,y,z){
    let el = document.createElement('a-box')
    el.setAttribute('width', x * 1.01)
    el.setAttribute('height', y * 1.01)
    el.setAttribute('depth', z * 1.01)
    el.setAttribute('physx-no-collision','')  
    el.setAttribute('material',{
        wireframe: true,
        color: 'red'
  ...
  //TODO: IMPLEMENT HULLS AND MESHES
  createDebugHull(obj3D){
    let el = document.createElement('a-entity')  
    el.setObject3D('mesh', new THREE.BoxHelper( obj3D.clone(), 0xff0000 ))  
    this.addHelper(el)
    el.setAttribute('scale', '1.01 1.01 1.01')
    el.setAttribute('physx-no-collision','') 
    /*  
    console.log(obj3D)
    
    el.setObject3D('mesh', obj3D.clone()); 
    ...
    */
    //el.object3D.add(new THREE.AxesHelper(0.5))
    this.el.appendChild(el) 
    this.debug = true;
  },    
    ...
    Util.traverseCondition(this.el.object3D,
      ...
      if (o.geometry) {
        ...
        else
        {
            ...
            /*
            if (this.data.shape == 'hull') {   
                //this.createDebugHull(this.el.object3D)
                geometry = this.createConvexMeshGeometry(o, this.el.object3D);
            } else {
                geometry = this.createTriangleMeshGeometry(o, this.el.object3D);
            }
            */
        }
        if (!geometry) {
...
AFRAME.registerComponent('physx-ring', {
    //dependencies: ['physx-body'], 
    schema: {
        type: {default: 'static', oneOf: ['dynamic', 'static', 'kinematic']},
        radius: {type: 'number', default: 20},
        height: {type: 'number', default: 3},
        segments: {type: 'number', default: 16, min: 4},
        thetaLength: {type: 'number', default: 360},
        visible: {type: 'boolean', default: false}
    },
    // create box geometries
    // rotate box geometries as a ring
    // apply physx bodies to them
    // main element that listen to events
    init(){
        this.theta = this.data.thetaLength * Math.PI / 180;
        this.perimeter = 2 * Math.PI * this.data.radius;
        
        for (let i=0;i<this.data.segments;i++){
            this.el.appendChild(this.createBox(i))
        }    
    },
    createBox: (function(){
        let spherical = new THREE.Spherical();
        ...
        return function(i) {
            ...
            box.setAttribute('height',this.perimeter/this.data.segments)
            box.setAttribute('width',this.data.height)
            box.setAttribute('depth',0.01)
            box.setAttribute('material','wireframe',true)
            box.setAttribute('visible',this.data.visible)
            box.setAttribute('physx-body','type',this.data.type)
        
            //positioning box
            spherical.radius = this.data.radius;
            spherical.theta = i * this.theta / this.data.segments;
            
            euler.set((spherical.theta-1.57),0,0);
            
            box.object3D.position.setFromSphericalCoords(spherical.radius, spherical.theta, spherical.phi)
            box.object3D.quaternion.setFromEuler(euler)
...
AFRAME.registerComponent('physx-joint-driver', {
  ...
  setJointDriver() {
    ...
    if (dev) console.log("Setting joint driver", this.driveAxes, this.enumAxes)
...
AFRAME.registerComponent('physx-joint-constraint', {
  ...
  setJointConstraint() {
    if (this.el.components['physx-joint__r'].data.type !== 'D6') {
    ...
    let joint = this.el.components['physx-joint__r'].joint;
