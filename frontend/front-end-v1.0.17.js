...
function headScript() {
    ...
    scriptEl.onload = ()=>{
        ...
        if (src.includes('/aframe-v')) THREE.Math = THREE.MathUtils;
        (src.includes('oorbit')) ? loadGroup(mode,iat,progress) : headScript(arr);       
    }
    document.head.appendChild(scriptEl);
}

var inspurl =  js+'/aframe-inspector-v'+version.INSPECTOR+ssuf;
...
if (bc == 83 || bc == 84) {
    ...
    arr.push('https://cdn.jsdelivr.net/gh/AR-js-org/AR.js@'+version.ARPLACE+'/aframe/build/aframe-ar.js');
}
//TODO: AR 81 E 82

if (a == "o") arr.push("https://cdn.jsdelivr.net/npm/aframe-orbit-controls@1.3.0/dist/aframe-orbit-controls.min.js")
...
var physxwasm = spre+'physx-v'+version.PHYSXW+'.wasm';
...
if (idev) {
    arr.push(js+'/components-a'+version.OORBIT+ssuf);
    arr.push(js+'/oorbit-a'+version.OORBIT+ssuf);    
} 
else {
    arr.push('../'+js+'/components-a'+version.OORBIT+ssuf);
    arr.push('../'+js+'/oorbit-a'+version.OORBIT+ssuf);    
...
var o, s3path, path, ap, error, url, logo, orb, geo, hide, exer;
var seats, form;
...
function getWorld(){                
    ...
    if (iat == 'GOOG' || iat == 'FACE' || iat == 'COG1' || iat == 'COG2') {
        ...
        if (urlParams.get('e') || document.referrer.indexOf(pass) > -1 || mode == AWS.config.credentials.identityId) {
